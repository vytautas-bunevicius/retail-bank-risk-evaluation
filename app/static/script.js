...
function validateForm() {
    ...
    const loanTermYears = parseInt(document.getElementById('loan_term_years').value, 10);
...
function displayResult(prediction) {
    ...
    if (prediction.loan_approval === "Approved") {
        ...
        details = `
            <p><strong>Monthly Payment:</strong> $${prediction.monthly_payment.toLocaleString()}</p>
            <p><strong>Probability of Default:</strong> ${percentage}%</p>
            <p><strong>Reason:</strong> ${prediction.rejection_reason || 'Risk level classified as High.'}</p>
        `;
    }

    if (prediction.anomalies_detected && prediction.anomalies_details.length > 0) {
        details += `<h4>Anomalies Detected:</h4><ul>`;
        prediction.anomalies_details.forEach(anomaly => {
            details += `<li><strong>${anomaly.feature}:</strong> $${anomaly.value.toLocaleString()} (Allowed: $${anomaly.lower_bound.toLocaleString()} - $${anomaly.upper_bound.toLocaleString()})</li>`;
        });
        details += `</ul>`;
    }
    ...
    if (prediction.loan_approval === "Approved") {
        resultModal.querySelector('.modal-content').style.borderTop = `5px solid var(--success-color)`;
    } else {
        resultModal.querySelector('.modal-content').style.borderTop = `5px solid var(--error-color)`;
    }
...
function showLoadingModal() {
    ...
    resultModal.querySelector('.modal-content').style.borderTop = `5px solid var(--primary-color)`;

...
This project focuses on developing, deploying, and serving a machine learning
model for credit risk prediction using the Home Credit dataset. The goal is to
create an interpretable, deployable, and financially sound model that
effectively identifies potential loan defaulters while maintaining a balance
...
2. **Feature Engineering**:
    ...
    - Created financial ratios (debt-to-income, credit-to-goods,
      annuity-to-income)
...
While these metrics indicate that the final model may not be the best in terms
of raw performance, it's important to note that our manual fine-tuning process
has significantly improved the model's effectiveness on edge cases. The
incorporation of domain knowledge and financial best practices allows the model
to make more nuanced and accurate decisions in complex scenarios that may not be
well-represented in the general test set.

## Key Findings

1. The model demonstrates high recall (74.42%) for detecting defaults, crucial
   in credit risk management.
2. This high recall comes at the cost of low precision (11.23%), indicating a
   tendency to overpredict defaults.
3. The model errs on the side of caution, which may be acceptable if the cost of
   missing a default significantly outweighs the cost of false alarms.
4. The precision-recall curve suggests the model performs moderately well but is
   dealing with imbalanced data.
5. Manual fine-tuning improved the model's alignment with real-world financial
   decision-making processes, particularly for edge cases and complex scenarios.
...
**Note:** This project uses Python 3.10 as specified in the `.python-version`
file.
...
4. **Sync Dependencies:**
   ...
   This installs all dependencies from `pyproject.toml` and sets up the project
   for development.
...
3. **Install the Package in Editable Mode:**
   ...
   This installs the project and all its dependencies as defined in
   `pyproject.toml`.
...
By default, this will run the server on `http://127.0.0.1:8000`. If you need to
specify a different host or port, you can use the `--host` and `--port` options:
...
The application is deployed on Google Cloud Platform using Cloud Run. To deploy
your own instance:
...
This command uses the `cloudbuild.yaml` configuration file to build and deploy
the application, ensuring consistency and reproducibility in the deployment
process.
...
The application features a user-friendly interface for loan applications. Here
are some screenshots of the UI:
<p float="left">
...
6. Conduct more extensive testing on edge cases to quantify the improvements
   from manual fine-tuning
